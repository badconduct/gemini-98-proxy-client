...
function readProfile(userName) {
  ...
  if (fs.existsSync(filePath)) {
    try {
      ...
      ALL_PERSONAS.forEach((p) => {
        // Ensure the moderation object exists for the persona
        if (!worldState.moderation[p.key]) {
          worldState.moderation[p.key] = {};
        }
        // Set defaults for any missing fields for backward compatibility
        worldState.moderation[p.key] = {
          warning: worldState.moderation[p.key].warning || false,
          blocked: worldState.moderation[p.key].blocked || false,
          violenceWarning:
            worldState.moderation[p.key].violenceWarning || false,
        };
