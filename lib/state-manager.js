...
function generateInitialWorldState(
  ...
  const world = {
    ...
    userLikes: {}, // NEW: Stores topics the user likes
    userDislikes: {}, // NEW: Stores topics the user dislikes
  };
  ...
  ALL_PERSONAS.forEach((p) => {
    // All personas need a moderation object to prevent undefined errors.
    world.moderation[p.key] = { warning: false, blocked: false };

    // Check if the persona is a friend by seeing if it's in the FRIEND_PERSONAS list
    ...
    if (isFriend) {
      ...
      // It's a utility bot, bots don't have relationship scores.
