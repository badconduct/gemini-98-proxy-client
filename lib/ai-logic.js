 ...
 */
function generatePersonalizedInstruction(persona, worldState, isSummer) {
  const {
    realName,
    userName,
    userScores,
...
Your relationship score with them is 100/100. You are best friends and completely loyal.
...
Example: { "reply": "OMG remember that time we talked about Chrono Trigger for like an hour? good times...", "relationshipChange": 3, "isImageRequest": false }`;
  ...
  const bffs = FRIEND_PERSONAS.filter(
    (p) => worldState.userScores[p.key] === 100
  );
  let vouchContext = "";
  ...
  const friendData = friendsToInclude
    .map((friend) => {
      ...
      return `- ${friend.name} (${
        ...
        ", "
      )}. Your current relationship score with them is ${score}/100. Current Status: ${onlineStatus}`;
    })
    .join("\n");
...
INSTRUCTIONS: Use this data and the social context to answer questions about friends' interests, relationship scores, or their current online/offline status. If asked why someone is offline, use your knowledge of their schedule and the current time to give a plausible reason (e.g., 'Kevin is at football practice right now because it's a weekday afternoon during the school year.'). If asked about a relationship, provide a helpful, concise, single-line hint that explains the 'why'.`;
