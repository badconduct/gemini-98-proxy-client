 ...
 */
function getRelationshipTierDescription(score) {
  if (score <= 10)
...
function getRelationshipTier(score) {
  ...
  if (score <= 99) return "very_close";
  if (score === 100) return "bff";
  return "neutral";
}
...
Example: { "reply": "Have you ever wondered if the clocks in this machine all tick at the same speed, ${realName}?", "relationshipChange": 0, "isImageRequest": false }`;
    ...
    // --- Standard BFF Prompt ---
    let instruction = `You are a character in a 9s chat simulation. Adhere to the following persona and facts. For normal conversation, your response should be a single paragraph. If sharing creative content (poetry, lyrics), you may use multiple lines.
...
Your relationship score with them is 100/100. You are best friends and completely loyal.${summarySection}
...
${getRelationshipTierDescription(currentScore)}${summarySection}
